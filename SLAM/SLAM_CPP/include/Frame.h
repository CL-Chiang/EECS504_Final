...
public:
    ...
    Frame(const cv::Mat &imLeft, const cv::Mat &imRight,
            const std::vector<BBX> left_results, const std::vector<BBX> right_results,
    ...
    Frame(const cv::Mat &imGray, std::vector<BBX> ODBox, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth);
...
public:
    ...
    // Image pyramid for direct method
    ...
    std::vector<BBX> mODBox;

    // flag to indicate if the ORB features are detected
    bool mbFeatureExtracted = false;
