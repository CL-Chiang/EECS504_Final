...
public:
    ...
    std::vector<BBX> bbx;
    void set(int x, int y, int width, int height, int cls) {
        bbx.push_back(BBX(x, y, width, height, cls));
    }
};
...
public:
    ...
    cv::Mat TrackMonocular(const cv::Mat &im, std::vector<BBX> od_bbx, const double &timestamp);
