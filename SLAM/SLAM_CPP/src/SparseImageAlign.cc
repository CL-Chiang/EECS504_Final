...
namespace ORB_SLAM2 {
    ...
    size_t SparseImgAlign::run(Frame *ref_frame, Frame *cur_frame, SE3f &TCR) {
        ...
        if (ref_frame->mvKeys.empty()) {
            ...
            std::cout << "SparseImgAlign: no features to track!" << endl;
            return 0;
        }
        ...
        SE3f cur_frame_Tcw = Converter::toSE3f(cur_frame->mTcw);
        SE3f ref_frame_Tcw = Converter::toSE3f(ref_frame_->mTcw);

        SE3f T_cur_from_ref(cur_frame_Tcw * ref_frame_Tcw.inverse());
    ...
    void SparseImgAlign::precomputeReferencePatches() {
        ...
        for (int i = 0; i < ref_frame_->N; i++, ++feature_counter) {
            ...
            SE3f ref_frame_Twc = Converter::toSE3f(ref_frame_->mTcw);
            Vector3f mp_WorldPose = Vector3f(mp->GetWorldPos().at<float>(0),mp->GetWorldPos().at<float>(1),mp->GetWorldPos().at<float>(2));
            const Vector3f xyz_ref = ref_frame_Twc * mp_WorldPose;
            //const Vector3f xyz_ref = ref_frame->mTcw * mp->GetWorldPos();
