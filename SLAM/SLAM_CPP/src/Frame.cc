...
Frame::Frame(const cv::Mat &imGray, std::vector<BBX> ODBox, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth)
    :mpORBvocabulary(voc),mpORBextractorLeft(extractor),mpORBextractorRight(static_cast<ORBextractor*>(NULL)),
     mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth),
     mImGray(imGray.clone()), mSensor(Monocular)
{
...
void Frame::ExtractORB(int flag, const cv::Mat &im)
{
    if (flag == 0) {
        ...
        (*mpORBextractorRight)(this, mvKeysRight, mDescriptorsRight, false);
    }
}
...
void Frame::ComputeImagePyramid() {
    // copy pyramid to frame for direct method
