...
void ORBmatcher::GetWarpAffineMatrix(
        ...
        const Vector3f pt_world = Converter::toVector3f(mp->GetWorldPos());
        const SE3f ref_pose = Converter::toSE3f(ref->GetPose());
...
bool ORBmatcher::FindDirectProjection(
    ...
    SE3f pose_ref = Converter::toSE3f(ref->GetPose());
    SE3f TCR = Converter::toSE3f(curr->mTcw) * pose_ref.inverse();

    // Compute affine wrap with border
    ...
    bool success = ORB_SLAM2::Align2D(curr->mvImagePyramid[search_level], _patch_with_border, _patch, 10, px_scaled);
